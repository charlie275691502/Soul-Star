...
public class Enemys_001 : AI {

	override public void specific_AI(){
		if (gameController.acts.round == 1) {
			set_skill ("前進", 0);
			priority_dir [0] = random_dir (0);
			set_skill ("前進", 1);
			priority_dir [1] = random_dir (3);
			set_skill ("橫斬", 2);
			priority_dir [2] = random_dir (3);
			set_skill ("突刺", 3);
			priority_dir [3] = random_dir (2);
			set_skill ("普通攻擊", 4);
			priority_dir [4] = random_dir (2);
			return;
		}
