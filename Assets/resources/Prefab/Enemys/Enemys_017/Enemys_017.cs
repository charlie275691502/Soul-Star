		...
		if (nearest_allies_vec.z >= 5) {
			...
			for (int i = 1; i < 5; i++) {
				...
				if (skills [i].GetComponent<Click_skill>().Name == "Forward") {
					...
					priority_dir [i] = direct_ration_random (nearest_allies_vec.x, nearest_allies_vec.y);
				} else if (skills [i].GetComponent<Click_skill>().Name == "Thrust") {
			...
			if (Mathf.Abs (nearest_allies_vec.x) == 2 || Mathf.Abs (nearest_allies_vec.y) == 2) {
				//第一格採(3:1)進行普通攻擊/Thrust
				set_skill ((random(0.75f)) ? "Attack" : "Thrust", 0);
			...
			//第一格以(6:3:1)進行橫斬/Attack/Thrust
			set_skill ((random(0.6f)) ? "Swing" : (random(0.75f)) ? "Attack" : "Thrust", 0);

			priority_dir [0] = random_dir (face_dir(nearest_allies_vec));

			//若為突刺
			if(skills[0].GetComponent<Click_skill>().Name == "Thrust"){
				//擇後三格隨機放上Forward*2、橫斬
